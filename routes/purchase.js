...
router.post('/purchasers', async (req, res) => {
  ...
  try {
    const result = await pool.query(
      'INSERT INTO purchaser (purchaser_name) VALUES ($1) RETURNING *',
      [purchaser_name.trim()]
    );
    res.status(201).json(result.rows[0]);
  } catch (err) {
    console.error('Error adding purchaser:', err);
...
router.delete('/purchasers/:id', async (req, res) => {
  ...
  try {
    const result = await pool.query('DELETE FROM purchaser WHERE id = $1 RETURNING *', [id]);

    if (result.rowCount === 0) {
      return res.status(404).json({ error: 'Purchaser not found' });
    }

    res.json({ message: 'Purchaser deleted successfully' });
  } catch (err) {
    console.error('Error deleting purchaser:', err);
    res.status(500).json({ error: 'Failed to delete purchaser' });
  }
});
  ...
  try {
    const result = await pool.query(
      `INSERT INTO purchase_entry
        (party, date, purchaser, item, quantity, unit, category, reference_bill_number, submit)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, 'yes')
       RETURNING *`,
      [party, date, purchaser, item, quantity, unit, category, reference_bill_number]
    );
    res.json({ message: 'Entry added', entry: result.rows[0] });
  } catch (error) {
...
router.get('/not-billed', async (req, res) => {
  try {
    ...
    console.error('Error fetching Not Billed entries:', err);
    res.status(500).json({ error: 'Internal server error' });
  }
});
...
router.patch('/entry-to-billed/:id', async (req, res) => {
  const id = req.params.id;
  ...
  try {
    ...
    res.json({ message: 'Billed successfully', data: result.rows[0] });
  } catch (err) {
    console.error('Error updating approval:', err);
    res.status(500).json({ error: 'Internal server error' });
  }
